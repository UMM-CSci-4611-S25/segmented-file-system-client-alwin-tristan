...
@test "Your client correctly assembled small.txt" {
  ...
  run diff tests/target-files/small.txt src/small.txt

  [ "$status" -eq 0 ]
}

@test "Your client correctly assembled AsYouLikeIt.txt" {
  run diff tests/target-files/AsYouLikeIt.txt src/AsYouLikeIt.txt

  [ "$status" -eq 0 ]
}

@test "Your client correctly assembled binary.jpg" {
  run diff tests/target-files/binary.jpg src/binary.jpg
